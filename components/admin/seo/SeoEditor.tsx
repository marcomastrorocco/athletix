...
import { Save, Search, Share2, Code2, SlidersHorizontal } from "lucide-react";
import type {
  PageSeo,
  SchemaType,
  SocialMeta,
...
export default function SeoEditor({
  ...
  const set = (patch: Partial<PageSeo>) => setSeo((s) => ({ ...s, ...patch }));
  const setOg = (patch: Partial<SocialMeta>) =>
    setSeo((s) => ({ ...s, og: { ...s.og, ...patch } }));
  const setTw = (patch: Partial<SocialMeta>) =>
    setSeo((s) => ({ ...s, twitter: { ...s.twitter, ...patch } }));
        ...
        {tab === "general" && (
          <div className="form-grid">
            <p className="field field-wide muted" style={{ marginBottom: 4, fontSize: 12 }}>
              Leave a field blank to keep the page&apos;s current default
        ...
        {tab === "social" && (
          <div className="form-grid">
            ...
            <label className="field field-wide">
              ...
              <input type="text" value={seo.og?.image ?? ""} onChange={(e) => setOg({ image: e.target.value })} placeholder="/image/og-cover.jpg or https://…" />
            </label>
